...
export const addExistingUserAsSchoolStaff = async (context: Context) => {
  try {
    console.log("hello");
  } catch (error: any) {
    return context.status(error.status || 500, { success: false, message: error.message || "Internal server error", code: error.code || "internal_server_error"});
